...
public class GradeServiceImpl implements GradeService{
	...
	public String searchGradeByHak(String hak) {
		...
		for(int i = 0; i < vc.size();i++){
			...
			// myHak 은 벡터에서 i번째 요소에서 학번을 리턴하여 담은 변수..타입은 String
			String myHak = vc.elementAt(i).getHak();

			// 파라미터로 받아온 hak과 벡터에 담겨있는 myHak이 일치하는지 여부를 체크하는 로직
	...
	public Vector<String> searchGradeByName(String name) {
		...
		for(int i = 0; i < vc.size();i++){
			...
			// myHak 은 벡터에서 i번째 요소에서 학번을 리턴하여 담은 변수..타입은 String
			String myName = vc.elementAt(i).getHak();
	...
	/*	 6- ascGradeTotal() //  오름차순이면 Arrays.sort 를 쓰면 되겠군.	 * */
	@Override
